...
```{r}
filter(full_results, adj.P.Val < 0.05, abs(logFC) > 1.5) %>%
  write_tsv(file = "/Users/anb/Documents/CMEB-Lab/ESCC-Drug-Repositioning/DR-escc-data/output/GSE23400/filtered_de_results_1-5fc_05pval.tsv")
```
...
df = read.csv("/Users/anb/Documents/CMEB-Lab/ESCC-Drug-Repositioning/DR-escc-data/output/GSE23400/filtered_de_results_1-5fc_01pval.tsv", sep="\t", header=TRUE)
```
...
```
```{r}
upreg <- df[df$logFC >= 1.5, ]
downreg <- df[df$logFC <= -1.5, ]
```
...
```{r}
write_tsv(as.data.frame(upreg_gene_list), file="/Users/anb/Desktop/upreg_gene_list_1-5fc_01pval.tsv", col_names = FALSE)
write_tsv(as.data.frame(downreg_gene_list), file="/Users/anb/Desktop/downreg_gene_list_1-5fc_01pval.tsv", col_names = FALSE)
```
